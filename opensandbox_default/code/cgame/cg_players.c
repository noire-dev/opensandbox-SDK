...
static qboolean CG_ParseAnimationFile(const char *filename, clientInfo_t *ci) {
	...
	for(i = 0; i < MAX_ANIMATIONS; i++) {
		...
		if(i == LEGS_WALKCR) skip = animations[LEGS_WALKCR].firstFrame - animations[TORSO_GESTURE].firstFrame;
		if(i >= LEGS_WALKCR && i < MAX_ANIMATIONS) animations[i].firstFrame -= skip;
...
static qboolean CG_RegisterClientSkin(clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *legsModelName, const char *legsSkinName) {
	...
	// if one skin loaded
	if(!ci->legsSkin && !ci->torsoSkin && !ci->headSkin && !ci->legsShader && !ci->torsoShader && !ci->headShader) return qfalse;
	return qtrue;
}

static qboolean CG_RegisterClientModelname(clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *legsModelName, const char *legsSkinName) {
	...
	if(!CG_RegisterClientSkin(ci, modelName, skinName, headName, headSkinName, legsModelName, legsSkinName)) return qfalse;
	...
	if(!ci->modelIcon) {
		...
		if(CG_FileExists(filename)) ci->modelIcon = trap_R_RegisterShaderNoMip(filename);
	}
	if(!ci->modelIcon) {
...
static qboolean CG_ReloadClientModelname(clientInfo_t *ci, const char *modelName, const char *skinName, const char *headModelName, const char *headSkinName, const char *legsModelName, const char *legsSkinName) {
	const char *headName;
...
static void CG_LoadClientInfo(int clientNum, clientInfo_t *ci) {
	...
	modelloaded = CG_RegisterClientModelname(ci, ci->modelName, ci->skinName, ci->headModelName, ci->headSkinName, ci->legsModelName, ci->legsSkinName);

	for(i = 0; i < MAX_CUSTOM_SOUNDS; i++) {
		if(!(s = cg_customSoundNames[i])) break;
		if(modelloaded) ci->sounds[i] = trap_S_RegisterSound(va("sound/player/%s/%s", ci->modelName, s + 1), qfalse);
		if(!ci->sounds[i]) ci->sounds[i] = trap_S_RegisterSound(va("sound/player/%s/%s", DEFAULT_MODEL, s + 1), qfalse);
	}

	// reset any existing players and bodies, because they might be in bad frames for this new model
	for(i = 0; i < MAX_GENTITIES; i++) {
		if(cg_entities[i].currentState.clientNum == clientNum && cg_entities[i].currentState.eType == ET_PLAYER) CG_ResetPlayerEntity(&cg_entities[i]);
	}
}
...
static void CG_ReloadClientInfo(int clientNum, clientInfo_t *ci) {
	...
	modelloaded = CG_ReloadClientModelname(ci, ci->modelName, ci->skinName, ci->headModelName, ci->headSkinName, ci->legsModelName, ci->legsSkinName);
}
...
void CG_NewClientInfo(int clientNum) {
	...
	if(!CG_ScanForExistingClientInfo(&newInfo)) CG_LoadClientInfo(clientNum, &newInfo);
...
void CG_ReloadPlayers(void) {
	...
	for(i = 0, ci = cgs.clientinfo; i < cgs.maxclients; i++, ci++)
		if(ci->infoValid) CG_ReloadClientInfo(i, ci);
}
...
static void CG_SetLerpFrameAnimation(clientInfo_t *ci, lerpFrame_t *lf, int newAnimation) {
	...
	if(newAnimation < 0 || newAnimation >= MAX_TOTALANIMATIONS) CG_Error("Bad animation number: %i", newAnimation);
...
static void CG_RunLerpFrame(clientInfo_t *ci, lerpFrame_t *lf, int newAnimation, float speedScale) {
	...
	if(newAnimation != lf->animationNumber || !lf->animation) CG_SetLerpFrameAnimation(ci, lf, newAnimation);
	...
	if(cg.time >= lf->frameTime) {
		...
		if(cg.time > lf->frameTime) lf->frameTime = cg.time;
	}
...
static void CG_PlayerAngles(centity_t *cent, vec3_t legs[3], vec3_t torso[3], vec3_t head[3]) {
	...
	if(clientNum >= 0 && clientNum < MAX_CLIENTS) {
		ci = &cgs.clientinfo[clientNum];
		if(ci->fixedtorso) torsoAngles[PITCH] = 0.0f;
		if(cg.renderingThirdPerson) torsoAngles[PITCH] = 0.0f;
		if(cg.renderingEyesPerson) torsoAngles[PITCH] = 0.0f;
	}
...
static void CG_HasteTrail(centity_t *cent) {
	...
	if(anim != LEGS_RUN && anim != LEGS_BACK) return;
...
static void CG_DustTrail(centity_t *cent) {
	...
	anim = cent->pe.legs.animationNumber & ~ANIM_TOGGLEBIT;
	if(anim != LEGS_LANDB && anim != LEGS_LAND) return;
	...
	if(!(tr.surfaceFlags & SURF_DUST)) return;
...
static void CG_PlayerTokens(centity_t *cent, int renderfx) {
	...
	if(tokens > MAX_SKULLTRAIL) tokens = MAX_SKULLTRAIL;
	...
	for(i = 0; i < trail->numpositions; i++) {
		...
		if(VectorNormalize(dir) > 30) VectorMA(origin, 30, dir, trail->positions[i]);
...
static void CG_PlayerPowerups(centity_t *cent, refEntity_t *torso) {
	...
	// quad gives a dlight
	if(powerups & (1 << PW_QUAD)) trap_R_AddLightToScene(cent->lerpOrigin, 200 + (rand() & 31), 0.2f, 0.2f, 1);
	...
	if(powerups & (1 << PW_HASTE)) CG_HasteTrail(cent);
}
...
void CG_Player(centity_t *cent) {
	...
	if(!legs.customSkin) legs.customShader = ci->legsShader;
	...
	if(!torso.customSkin) torso.customShader = ci->torsoShader;
	...
	if(cg.renderingEyesPerson) torso.renderfx &= RF_FIRST_PERSON;
	...
	if(cent->currentState.number != cg.snap->ps.clientNum && !(cent->currentState.powerups & (1 << PW_INVIS))) {
		if(cent->currentState.powerups & ((1 << PW_GUARD) | (1 << PW_SCOUT) | (1 << PW_DOUBLER) | (1 << PW_AMMOREGEN))) {
			memcpy(&powerup, &torso, sizeof(torso));
			if(cent->currentState.powerups & (1 << PW_GUARD)) powerup.hModel = cgs.media.guardPowerupModel;
			if(cent->currentState.powerups & (1 << PW_SCOUT)) powerup.hModel = cgs.media.scoutPowerupModel;
			if(cent->currentState.powerups & (1 << PW_DOUBLER)) powerup.hModel = cgs.media.doublerPowerupModel;
			if(cent->currentState.powerups & (1 << PW_AMMOREGEN)) powerup.hModel = cgs.media.ammoRegenPowerupModel;
			powerup.frame = 0;
			powerup.oldframe = 0;
	...
	if(cent->currentState.powerups & (1 << PW_INVULNERABILITY)) {
		if(!ci->invulnerabilityStartTime) ci->invulnerabilityStartTime = cg.time;
	...
	if(!head.customSkin) head.customShader = ci->headShader;
	...
	if(camereyes) {
		...
		if(cg.renderingEyesPerson) VectorCopy(cent->lerpAngles, headang);
...
void CG_ResetPlayerEntity(centity_t *cent) {
	...
	cent->pe.head.yawAngle = cent->lerpAngles[YAW];
	cent->pe.head.yawing = qfalse;
	cent->pe.head.pitchAngle = cent->lerpAngles[PITCH];
