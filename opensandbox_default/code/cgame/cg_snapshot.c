...
static void CG_ResetEntity(centity_t *cent) {
	...
	if(cent->snapShotTime < cg.time - EVENT_VALID_MSEC) cent->previousEvent = 0;

	cent->trailTime = cg.snap->serverTime;

	VectorCopy(cent->currentState.origin, cent->lerpOrigin);
	VectorCopy(cent->currentState.angles, cent->lerpAngles);
	if(cent->currentState.eType == ET_PLAYER) CG_ResetPlayerEntity(cent);
}
...
void CG_TransitionEntity(centity_t *cent) {
	...
	if(!cent->interpolate) CG_ResetEntity(cent);
...
static void CG_TransitionSnapshot(void) {
	...
	if(oldFrame) {
		...
		if(cg.demoPlayback || (cg.snap->ps.pm_flags & PMF_FOLLOW)) CG_TransitionPlayerState(ps, ops);
	}
}
...
static void CG_SetNextSnap(snapshot_t *snap) {
	...
	if(cg.nextSnap->ps.clientNum != cg.snap->ps.clientNum) cg.nextFrameTeleport = qtrue;

	// if changing server restarts, don't interpolate
	if((cg.nextSnap->snapFlags ^ cg.snap->snapFlags) & SNAPFLAG_SERVERCOUNT) cg.nextFrameTeleport = qtrue;
...
void CG_ProcessSnapshots(void) {
	...
	while(!cg.snap) {
		...
		if(!(snap->snapFlags & SNAPFLAG_NOT_ACTIVE)) CG_SetInitialSnapshot(snap);
	}
	...
	do {
		...
		if(cg.time >= cg.snap->serverTime && cg.time < cg.nextSnap->serverTime) break;
