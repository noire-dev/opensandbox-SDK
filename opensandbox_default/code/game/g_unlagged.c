...
static void G_TimeShiftClient(gentity_t *ent, int time, qboolean debug, gentity_t *debugger) {
	...
	do {
		if(ent->client->history[j].leveltime <= time) break;
	...
	if(j != k) {
		...
		if(j != ent->client->historyHead) {
			float frac = (float)(time - ent->client->history[j].leveltime) / (float)(ent->client->history[k].leveltime - ent->client->history[j].leveltime);
...
===================
*/
void G_PredictPlayerMove(gentity_t *ent, float frametime) { G_PredictPlayerStepSlideMove(ent, frametime); }
